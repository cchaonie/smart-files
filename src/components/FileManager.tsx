...
function MoveFileModal({
  ...
  const load = useCallback(async () => {
    ...
    setErr(null);
    try {
      const data = await fetchJson<{ folders: FolderRow[] }>(
        browseUrl(modalParentId),
      );
...
export function FileManager() {
  ...
  const loadBrowse = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchJson<{ folders: FolderRow[]; files: FileRow[] }>(
        browseUrl(currentParentId),
      );
      setFolders(data.folders);
      setFiles(data.files);
    } catch (e) {
  ...
  async function runUpload(file: File, itemId: number) {
    ...
    try {
      ...
      await fetchJson<{ file: { id: string } }>(
        `/api/upload/session/${uploadId}/complete`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // IMPORTANT: Copy files immediately before any async operation or value reset
    // because FileList is a live reference that gets cleared when input value is reset
    const files = fileList && fileList.length > 0 ? Array.from(fileList) : [];

    // Reset input immediately after copying files to allow re-selecting same file
    e.target.value = '';
